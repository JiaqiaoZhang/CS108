...
public class Piece {
	...
	*/
	...
	public Piece computeNextRotation() {
		TPoint[] rotateBody = new TPoint[body.length];
		for(int i = 0; i < body.length; i ++) {
			rotateBody[i] = new TPoint(height - 1 - body[i].y, body[i].x);		
		}
		return new Piece(rotateBody);
	}
	...
	*/
	private static Piece makeFastRotations(Piece root) {
		Piece currentPiece = root.computeNextRotation();
		Piece prePiece = root;
		root.next = currentPiece;
		while (!currentPiece.equals(root)) {
			prePiece = currentPiece;
			currentPiece.next = currentPiece.computeNextRotation();
	...
	*/
	...
	private static TPoint[] sortPoints(TPoint[] points) {
		for (int i = 0; i < points.length; i++) {
			for(int j = i; j < points.length; j ++) {
				if(points[i].x > points[j].x) {
					 TPoint tempPoint =points[i];
					 points[i] = points[j];
					 points[j] = tempPoint; 
				}
				if(points[i].x == points[j].x) {
